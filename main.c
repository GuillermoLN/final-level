...
int podeMover( int xPos, int yPos, char matriz[][N], int tecla, int linha, int coluna, BOMBA bomba[]);// Retorna 1 se o movimento eh valido
...
void interacoes(JOGADOR *jogador, BOMBA *bomba, CRIATURA *criatura, char mapa[][N], JOGO *jogo, int qtdCriatura); // Funcao que cuida de todas as interacoes entre os diferentes "objetos" do jogo

void desenhaMapa(char matriz[][N], JOGADOR jogador, CRIATURA criatura[], BOMBA bomba[], JOGO jogo,int cont) ; // funcao que aplica as estilizações as matrizes

void salvarJogo(char mapa[][N], JOGADOR jogador, CRIATURA criatura[], BOMBA bomba[], JOGO jogo, int qtdCriatura);

void lerEstadoJogo(JOGADOR *jogador, CRIATURA criatura[], BOMBA bomba[], JOGO *jogo, int qtdCriatura);

int leMapa(char nomeArq[], char mapa[][N], JOGADOR *jogador, CRIATURA *criatura);

void initJogo(JOGADOR *jogador, CRIATURA *criatura, BOMBA *bomba, JOGO *jogo, int qtdCriatura);

void controlePontos( int *pontos, int varPontos, int *vida);

int verifFimJogo(char mapa[][N]);
...
int main(void)
{
    ...
    srand(time(NULL));
    cont = leMapa( jogo.arquivo, matriz, &jogador, criatura);
    initJogo(&jogador, criatura, bomba, &jogo, cont);

    do{
        //Loop principal
        if(jogo.estado == JOGANDO){
                InitWindow(LARGURA, ALTURA + BORDA, "Jogo"); // Ajusta o titulo da janela
                SetTargetFPS(60);//Ajusta o jogo para executar a 60 quadros por segundo
                tempCriatura = GetTime();
            while (!WindowShouldClose() && jogo.estado == JOGANDO){// Detecta se deve continuar e faz o jogo acontecer
                ...
                interacoes(&jogador, bomba, criatura, matriz, &jogo, cont);

                desenhaMapa(matriz, jogador, criatura, bomba, jogo, cont);
                if(!verifFimJogo(matriz)){
                    nomeArquivo(jogo.arquivo);
                    leMapa(jogo.arquivo,matriz,&jogador,criatura);
                }
        ...
        if(jogo.estado == MENU){
            ...
            while(jogo.estado == MENU){
                ...
                case KEY_N:
                    jogo.estado = JOGANDO;
                    cont = leMapa("mapa1.txt",matriz,&jogador,criatura);
                    initJogo(&jogador, criatura, bomba, &jogo, cont);
                    break;
                case KEY_V:
                    jogo.estado = JOGANDO;
                    break;
                case KEY_S:
                    salvarJogo(matriz,jogador,criatura,bomba, jogo, cont);
                    break;
                case KEY_Q:
                    jogo.estado = FIM;
                    break;
                case KEY_C:
                    leMapa("save.txt", matriz, &jogador, criatura);
                    lerEstadoJogo(&jogador, criatura, bomba, &jogo, cont);
                    jogo.estado = JOGANDO;
                    break;
...
void desenhaMapa(char matriz[][N],  JOGADOR jogador, CRIATURA criatura[], BOMBA bomba[], JOGO jogo, int cont){
    int i = 0,j = 0;
    int posX=0, posY=0;
    char texto[N];
    BeginDrawing();
    ClearBackground(RAYWHITE);//Limpa e define uma cor de fundo
        while(strlen(matriz[i])){
            j = 0;
            while(matriz[i][j] != '\0'){
                switch (matriz[i][j]){
                case 'W':
                ...
                case 'D':
                    ...
                    break;
                case ' ':
                    DrawRectangle(j * ARESTA, i * ARESTA, ARESTA, ARESTA, BLACK);
                    break;
                }
                j++;
            }
       ...
       for(i = 0; i < cont; i++)
        ...
        DrawText( texto, BORDA/2, 40 * LINHA + BORDA/2, BORDA/2, BLACK);
...
int podeMover( int xPos, int yPos, char matriz[][N], int tecla, int linha, int coluna, BOMBA bomba[])
{
    int flag_mov = 1, i;
...
void interacoes(JOGADOR *jogador, BOMBA *bomba, CRIATURA *criatura, char mapa[][N], JOGO *jogo, int qtdCriatura)
{
    int i = 0, j, contBomba;
    char mapaCpy [LINHA][N];

    while(strlen(mapa[i])){
        strcpy(mapaCpy[i],mapa[i]);
        i++;
    }
        ...
        if(bomba[contBomba].efeito){// verificacao de colisao da explosao
            for(i = 0; i < EXPLOSAO * EXPLOSAO; i++){
                ...
                if(!verifColisCord(bomba[contBomba].xExp[i],bomba[contBomba].yExp[i], jogador->xPos, jogador->yPos)){ // Se a bomba explodir no jogador
                    jogo -> vida--;
                    leMapa("mapa1.txt", mapaCpy, jogador, criatura);
                }
...
void initJogo(JOGADOR *jogador, CRIATURA *criatura, BOMBA *bomba, JOGO *jogo, int qtdCriatura)
{
    ...
    for(i = 0; i < qtdCriatura; i++){ // Zerando o contador de movimento
        criatura[i].movCont = 0;
        criatura[i].direcao = rand() % 4 + KEY_RIGHT;
        criatura[i].vida = 1;
    }
